...
from shinka.core.judge_gate import AdversarialJudgeGate
from shinka.core.judge_instrumentation import JudgeInstrument
...
class ShinkaEvolveRunner:
    ...
    def __init__(
        ...
        # generator. Off unless judge_gate_enabled; falls back to the novelty
        # model family when adversarial_judge_models is unset.
        self.judge_gate = None
        self.judge_instrument = None
        if getattr(evo_config, "judge_gate_enabled", False):
            judge_models = (
                evo_config.adversarial_judge_models or evo_config.novelty_llm_models
            )
            if judge_models:
                judge_llm = AsyncLLMClient(
                    model_names=judge_models,
                    **_llm_kwargs_with_headless_work_dir(
                        evo_config.adversarial_judge_kwargs,
                        Path(self.results_dir),
                    ),
                )
                self.judge_gate = AdversarialJudgeGate(
                    async_llm_client=judge_llm,
                    thresholds=evo_config.adversarial_thresholds,
                    language=evo_config.language,
                )
                self.judge_instrument = JudgeInstrument(self.results_dir)
                logger.info(
                    f"Adversarial judge gate ENABLED with models: {judge_models}"
                )
            else:
                logger.warning(
                    "judge_gate_enabled=True but no adversarial_judge_models or "
                    "novelty_llm_models configured; judge gate disabled."
                )
    ...
    async def _generate_evolved_proposal(
        ...
        if proposal_accepted:
            ...
            # ===== PRE-EVAL ADVERSARIAL JUDGE GATE (§6 funnel) =====
            # A cheap, decorrelated pre-filter that decides whether this candidate
            # is worth an expensive GROUNDED verifier call. It is never
            # authoritative about correctness — the verifier is the source of
            # truth. It fails OPEN so a flaky judge cannot starve the loop, and
            # every verdict is recorded for the false-reject audit (§9).
            if generation > 0 and getattr(self, "judge_gate", None) is not None:
                judge_verdict = await self.judge_gate.assess(
                    exec_fname=exec_fname,
                    code_diff=code_diff,
                    parent_program=parent_program,
                )
                # Judge cost tracked like novelty cost (counted once for both the
                # accept path below and the reject path's terminal-failure record).
                novelty_total_cost += judge_verdict.cost
                meta_patch_data["judge_verdict"] = judge_verdict.as_dict()
                if getattr(self, "judge_instrument", None) is not None:
                    self.judge_instrument.record(
                        generation=generation,
                        parent_id=(parent_program.id if parent_program else None),
                        verdict=judge_verdict,
                    )
                if not judge_verdict.admit:
                    await self._record_terminal_failed_proposal(
                        ...
                        failure_stage="judge",
                        failure_reason=(
                            judge_verdict.strongest_weakness
                            or "rejected by adversarial judge gate"
                        ),
                    )
                    return None
            # ===== END JUDGE GATE =====
