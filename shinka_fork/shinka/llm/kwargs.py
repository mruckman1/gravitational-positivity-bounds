...
def sample_model_kwargs(
    ...
    # 2.pre. Claude-5-family via OpenRouter: the generic reasoning.effort
    # parameter is SILENTLY IGNORED (adaptive-thinking models); depth is
    # controlled by the top-level `verbosity` field (low/medium/high/
    # xhigh/max). Route through extra_body so the OpenAI SDK forwards it.
    _CLAUDE5_TOKENS = ("claude-sonnet-5", "claude-opus-4-8", "claude-fable-5",
                       "fable-5", "claude-haiku-4-5")
    if provider == "openrouter" and any(t in api_model_name
                                        for t in _CLAUDE5_TOKENS):
        kwargs_dict["temperature"] = (
            1.0 if has_fixed_temperature(api_model_name)
            else random.choice(temperatures))
        kwargs_dict["max_output_tokens"] = random.choice(max_tokens)
        r_effort = random.choice(reasoning_efforts) if reasoning_efforts \
            else ""
        if r_effort and r_effort != "disabled":
            kwargs_dict["extra_body"] = {"verbosity": r_effort}
        return kwargs_dict
